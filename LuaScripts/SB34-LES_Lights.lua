...
-- NAV light
ref_nav_light_pos = XPLMFindDataRef("les/sf34a/acft/ltng/anm/navigation_lights_switch")

function turn_on_nav_light()
    local current_pos = XPLMGetDatai(ref_nav_light_pos)
    if (current_pos == 0) then        
        command_once("les/sf34a/acft/ltng/mnp/navigation_lights_switch")
        XPLMSetDatai(ref_nav_light_pos, 1)
    end
end

function turn_off_nav_light()
    local current_pos = XPLMGetDatai(ref_nav_light_pos)
    if (current_pos == 1) then
        command_once("les/sf34a/acft/ltng/mnp/navigation_lights_switch")
        XPLMSetDatai(ref_nav_light_pos, 0)
    end
end

-- Beacon light
ref_beacon_light_pos = XPLMFindDataRef("les/sf34a/acft/ltng/anm/beacon_lights_switch")

function turn_on_beacon_light()
    local current_pos = XPLMGetDatai(ref_beacon_light_pos)
    if (current_pos == 0) then        
        command_once("les/sf34a/acft/ltng/mnp/beacon_lights_switch")
        XPLMSetDatai(ref_beacon_light_pos, 1)
    end
end

function turn_off_beacon_light()
    local current_pos = XPLMGetDatai(ref_beacon_light_pos)
    if (current_pos == 1) then
        command_once("les/sf34a/acft/ltng/mnp/beacon_lights_switch")
        XPLMSetDatai(ref_beacon_light_pos, 0)
    end
end
...
if string.sub(AIRCRAFT_FILENAME, 1, 13) == "LES_Saab_340A" then
    ...
    create_command("FlyWithLua/finlayc/Saab340/nav_light_on", "turn nav light on",
    "turn_on_nav_light()", "", "")
    
    create_command("FlyWithLua/finlayc/Saab340/nav_light_off", "turn nav light off",
    "turn_off_nav_light()", "", "")

    create_command("FlyWithLua/finlayc/Saab340/beacon_light_on", "turn beacon light on",
    "turn_on_beacon_light()", "", "")
    
    create_command("FlyWithLua/finlayc/Saab340/beacon_light_off", "turn beacon light off",
    "turn_off_beacon_light()", "", "")
